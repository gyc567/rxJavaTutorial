...
class TheConsumer implements Runnable {
    ...
    public TheConsumer(BlockingQueue<Task> buffer) {
        this.buffer = buffer;
    }
...
class TheProducer implements Runnable {
    ...
    public TheProducer(BlockingQueue<Task> buffer) {
        this.buffer = buffer;
    }
...
public class ProducerConsumerInJavaUsingBlockingQueue {
    ...
    public static void main(String[] args) {
        ...
        for(int i = 1; i <= Constants.NUM_OF_PRODUCER; ++i) {
            es.execute(new TheProducer(buffer));
        }
        for(int i = 1; i <= Constants.NUM_OF_CONSUMER; ++i) {
            es.execute(new TheConsumer(buffer));
        }
    }
}