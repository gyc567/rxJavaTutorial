...
class ListenerThread implements Runnable {
    ...
    private void handleAcceptableEvent(SelectionKey key) throws IOException {
    ...
    private void handleWritableEvent(ByteBuffer resp, SelectionKey key) throws IOException {
        // we are ready to send a response to one of the client sockets
        // we had read a command from previously
        System.out.println("Sending response!");
        SocketChannel ch = (SocketChannel)key.channel();

        System.out.println("the resp msg is "+resp.toString());
        ch.write(resp);
        resp.rewind();
        ...
        ch.register(this.sel, SelectionKey.OP_ACCEPT);
    }
}
...
public class JServer {
    public static void main (String[] args) {
        ...
        Selector sel = null;
        System.out.println("JServer is staring now ....");

        try {
            sch = getServerSocketChannel();
    ...
    private static void fireUpListener(Selector sel) {
        // fire up the listener thread, pass it our selector
        ListenerThread listener = new ListenerThread(sel);
        ExecutorService executor = Executors.newCachedThreadPool();
        executor.submit(listener);
        System.out.println("Server is up,listening on "+ HttpProtocol.HOST+":"+HttpProtocol.PORT);
    }

    private static ServerSocketChannel getServerSocketChannel() throws IOException {
        ServerSocketChannel sch;// setup the socket we're listening for connections on.
        InetSocketAddress addr = new InetSocketAddress(HttpProtocol.PORT);
